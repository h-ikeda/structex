defmodule Structex.Tensor do
  ...
  def new(order), do: %Structex.Tensor{tensor: order}
  ...
  def delete_key(%Structex.Tensor{tensor: %Tensorex{shape: shape}, index: index}, id)
      when is_map_key(index, id) and map_size(index) <= 1 do
    new(length(shape))
  end

  def delete_key(%Structex.Tensor{tensor: order, index: index}, id)
      when is_map_key(index, id) and map_size(index) <= 1 do
    new(order)
  end

  def delete_key(%Structex.Tensor{tensor: %Tensorex{shape: shape} = tensor, index: index} = t, id)
      when is_map_key(index, id) do
    case Map.pop(index, id) do
      {{nil, [], _}, new_index} ->
        %{t | index: new_index}
  ...
  def delete_key(%Structex.Tensor{index: index} = t, id), do: %{t | index: Map.delete(index, id)}
  ...
  def put_key(%Structex.Tensor{index: index} = t, id, degrees) when is_map_key(index, id) do
    put_key(delete_key(t, id), id, degrees)
  end
  ...
  def put_key(
    ...
    %{t | tensor: Tensorex.reshape(tensor, new_shape), index: Map.put(index, id, new_index)}
  end

  def put_key(%Structex.Tensor{tensor: order, index: index} = t, id, degrees) do
    case create_index(degrees, 0) do
      {new_index, 0} ->
        %{t | index: Map.put(index, id, new_index)}

      {new_index, range_size} ->
        shape = List.duplicate(range_size, order)
        %{t | tensor: Tensorex.zero(shape), index: Map.put(index, id, new_index)}
    end
  end
